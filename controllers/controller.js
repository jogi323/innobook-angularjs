...
exports.insertRequest = function(req,res){
	UserRegister.find({"_id" : req.params.id},function(req,res){
    var email = res[0].email;
    var data = {
			'sent_by':req.sessionId,
			'sent_to':email,
			'accept':0
			};
...
exports.friendsAndRequests = function(req,res){
	...
	friendrequests.find({$and: [{'sent_to':req.body.username }, {'accept':0}]},function(err,accept_records){
    	...
    	for (var y = 0; y < friends_count ; y++){
    		var each_count = {};
    		if(accept_records[x].sent_by == req.body.username){
    			each_count[friend_name] = accept_records[x].sent_to;
    		}else{
    			each_count[friend_name] = accept_records[x].sent_by;
    		}
